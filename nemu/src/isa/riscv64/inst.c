...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
    ...
    INSTPAT("??????? ????? ????? 000 ????? 00100 11", addi   , I, R(dest) = src1 + src2); // 不应该是加立即数吗
    // addi:把符号位扩展的立即数加到寄存器 x[rs1]上, 结果写入x[rd],忽略算术溢出

    INSTPAT("??????? ????? ????? 000 ????? 11000 11", beq    , I, if (src1 == src2) s->pc += src1);
    //beq 是相等条件分支，rs1 和 rs2 的值相等时，把 pc 的值设置成当前值+偏移值；
    INSTPAT("??????? ????? ????? 001 ????? 11000 11", bne   , I, if (src1 != src2) s->pc += src1);
    ...
    INSTPAT("??????? ????? ????? 011 ????? 00100 11", sliu   , I, if (src1 < src2) R(dest) = 1; else R(dest) = 0);
    //sltiu: rs1 小于立即数则 rd 置 1，否者置 0(无符号比较)

    INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, s->dnpc = s->pc + src1, R(dest) = s->snpc);
