...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
    ...
    INSTPAT("0000000 ????? ????? 101 ????? 00100 11", srli   , I, R(dest) = S64(src1) << src2);                                 // srli:立即数逻辑右移，rs1 右移 shamt 位，空位填 0 并写入rd 中；
    ...
    INSTPAT("??????? ????? ????? 100 ????? 00000 11", lbu    , I, R(dest) = Mr(src1 + src2, 4));                                // lbu: 是无符号字节加载，读取一个字节写入 rd 中；
