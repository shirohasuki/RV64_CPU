...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
    ...
    INSTPAT("??????? ????? ????? 011 ????? 01000 11", sd     , S, Mw(src1 + dest, 8, src2)); // 存储双字指令,8个字节
    ...
    INSTPAT("??????? ????? ????? 111 ????? 00100 11", andi   , I, R(dest) = src1 & immI(INSTPAT_INST(s))); // andi是 rs1 和立即数位与并写入 rd 中；
    INSTPAT("??????? ????? ????? 100 ????? 00100 11", xori   , I, R(dest) = src1 ^ immI(INSTPAT_INST(s))); // xori是立即数异或，rs1和立即数按位异或并写入 rd中；
    INSTPAT("??????? ????? ????? 110 ????? 00100 11", ori    , I, R(dest) = src1 | immI(INSTPAT_INST(s))); // ori是 rs1 和立即数按位取或并写入 rd 中；
    INSTPAT("??????? ????? ????? 000 ????? 00110 11", addiw  , I, R(dest) = src1 + immI(INSTPAT_INST(s))); //结果截断为32位是什么？ // addiw:把 立即数 加到寄存器 x[rs1]上, 将结果截断为 32 位, 把符号位扩展的结果写入 x[rd]

    INSTPAT("0000000 ????? ????? 001 ????? 00100 11", slli   , I, R(dest) = src1 << src2); // slli:立即数逻辑左移，rs1 左移 shamt 位，空位填 0 并写入 rd 中；
    INSTPAT("0000000 ????? ????? 101 ????? 00100 11", srli   , I, R(dest) = src1 >> src2); // srli:立即数逻辑右移，rs1 右移 shamt 位，空位填 0 并写入rd 中；
    INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I, R(dest) = src1 >> src2); // srai:立即数算数右移，rs1 右移 shamt 位，空位填 rs1 的最高位并写入 rd 中；

    INSTPAT("??????? ????? ????? 011 ????? 00000 11", ld     , I, R(dest) = Mr(src1 + src2, 8));  // 加载双字指令
    // INSTPAT("??????? ????? ????? 000 ????? 00000 11", lb     , I, R(dest) = src1 + src2); // lb: 字节加载，读取一个字节写入 rd 中；
    // INSTPAT("??????? ????? ????? 001 ????? 00000 11", lh     , I, R(dest) = src1 + src2); // lh: 是半字加载，读取两个字节写入 rd 中；
    INSTPAT("??????? ????? ????? 010 ????? 00000 11", lw     , I, R(dest) = src1 + src2); // lw: 是字加载，读取四个字节写入 rd 中；
    INSTPAT("??????? ????? ????? 100 ????? 00000 11", lbu    , I, R(dest) = src1 + src2); // lbu: 是无符号字节加载，读取一个字节写入 rd 中；
    // INSTPAT("??????? ????? ????? 101 ????? 00000 11", lhu    , I, R(dest) = src1 + src2); // lhu: 是无符号半字加载，读取两个字节写入 rd中；
    ...
    INSTPAT("??????? ????? ????? 110 ????? 11000 11", bltu   , B, if ((unsigned int)src1 < (unsigned int)src2)  s->dnpc = s->pc + immB(INSTPAT_INST(s))); //bltu 是无符号小于条件分支；
    INSTPAT("??????? ????? ????? 111 ????? 11000 11", bgeu   , B, if ((unsigned int)src1 >= (unsigned int)src2) s->dnpc = s->pc + immB(INSTPAT_INST(s))); //bgeu 是无符号大于等于条件分支；
    ...
    INSTPAT("??????? ????? ????? 001 ????? 01000 11", sh     , S, Mw(src1 + dest, 2, src2)); //sh 是存半字，把 rs2的低位两字节存入地址 rs1 + 立即数中    
    //sw 是存字，把 rs2 的低位四字节存入地址 rs1 + 立即数中
    INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu   , I, if (((unsigned int)src1 < (unsigned int)immI(INSTPAT_INST(s)))) R(dest) = 1; else R(dest) = 0); // sltiu: rs1 小于立即数则 rd 置 1，否者置 0(无符号比较)

    INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, s->dnpc = s->pc + immJ(INSTPAT_INST(s)), R(dest) = s->pc + 4); // jal:跳转并链接，把 pc 设置成当前值+偏移值，然后将 pc+4 当做下一条指令的地址存入 rd 中
