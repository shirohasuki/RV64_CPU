...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
    ...
    INSTPAT("0000000 ????? ????? 001 ????? 00100 11", slli   , I, R(dest) = SEXT(BITS(src1 << BITS(src2, 4, 0), 31, 0), 32)); // slli:立即数逻辑左移，rs1 左移 shamt 位，空位填 0 并写入 rd 中；
    ...
    // S-Type
    INSTPAT("??????? ????? ????? 000 ????? 01000 11", sb     , S, Mw(src1 + dest, 1, src2)); //sb 是存字节，把 rs2 的低位一字节存入地址 rs1+立即数中
    INSTPAT("??????? ????? ????? 001 ????? 01000 11", sh     , S, Mw(src1 + dest, 2, src2)); //sh 是存半字，把 rs2的低位两字节存入地址 rs1 + 立即数中    
    INSTPAT("??????? ????? ????? 010 ????? 01000 11", sw     , S, Mw(src1 + dest, 4, src2)); //sw 是存字，把 rs2 的低位四字节存入地址 rs1 + 立即数中
