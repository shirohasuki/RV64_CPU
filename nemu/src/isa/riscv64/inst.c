...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
    ...
    INSTPAT("??????? ????? ????? 000 ????? 00100 11", addi   , I, R(dest) = src1 + immI(INSTPAT_INST(s)); printf("%lx", immI(INSTPAT_INST(s)))); 
    ...
    INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(dest) = s->pc + 4, s->dnpc += s->pc + immJ(INSTPAT_INST(s)));
