...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
    ...
    INSTPAT("??????? ????? ????? 000 ????? 11000 11", beq    , B, if (src1 == src2) s->dnpc = s->pc + immB(INSTPAT_INST(s)));
    //beq 是相等条件分支，rs1 和 rs2 的值相等时，把 pc 的值设置成当前值+偏移值；
    INSTPAT("??????? ????? ????? 001 ????? 11000 11", bne    , B, if (src1 != src2) s->dnpc = s->pc + immB(INSTPAT_INST(s))); 
    ...
    INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, s->dnpc = (immI(INSTPAT_INST(s)) + src1) & (0), R(dest) = s->pc + 4);
    ...
    INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu   , I, if (src1 < immI(INSTPAT_INST(s) & (0))) R(dest) = 1; else R(dest) = 0);
