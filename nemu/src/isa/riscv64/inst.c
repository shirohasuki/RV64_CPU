...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
    ...
    INSTPAT("??????? ????? ????? 010 ????? 01000 11", sw     , S, Mw(src1 + dest, 4, BITS(src2, 15, 0)));                                    // sw 是存字，把 rs2 的低位四字节存入地址 rs1 + 立即数中
    INSTPAT("??????? ????? ????? 011 ????? 01000 11", sd     , S, Mw(src1 + dest, 8, BITS(src2, 31, 0)));                                    // sd: 存储双字指令,8个字节
