...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
    ...
    INSTPAT("??????? ????? ????? 001 ????? 11000 11", bne    , B, if (src1 != src2) s->dnpc = s->pc + immB(INSTPAT_INST(s)) - 4); //bne 是不等条件分支，rs1 和 rs2 的值不等时，把 pc 的值设置成当前值+偏移值；
    INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt    , B, if (src1 < src2)  s->dnpc = s->pc + immB(INSTPAT_INST(s)) - 4); //blt 是小于条件分支，rs1 小于 rs2 的值时，把 pc 的值设置成当前值+偏移值；
    INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , B, if (src1 >= src2) s->dnpc = s->pc + immB(INSTPAT_INST(s)) - 4); //bge是大于等于条件分支，rs1 大于等于 rs2 的值时，把 pc 的值设置成当前值+偏移值；
    INSTPAT("??????? ????? ????? 110 ????? 11000 11", bltu   , B, if ((unsigned int)src1 < (unsigned int)src2)  s->dnpc = s->pc + immB(INSTPAT_INST(s)) - 4); //bltu 是无符号小于条件分支；
    INSTPAT("??????? ????? ????? 111 ????? 11000 11", bgeu   , B, if ((unsigned int)src1 >= (unsigned int)src2) s->dnpc = s->pc + immB(INSTPAT_INST(s)) - 4); //bgeu 是无符号大于等于条件分支；
    ...
    INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu   , I, if ((src1 - immI(INSTPAT_INST(s))) < 0) R(dest) = 1; else R(dest) = 0); // may not right 
