...
class Ctrl extends Module {
    ...
    // 给事件进行优先编码
    val event_code = PriorityEncoder(Cat(NOEVENT, jump, axi_busy | axi_busy_start, load_data_hit)) // 从低到高输出第一个有1的位数 0->NOEVENT
    ...
    val flush_list  = ListLookup(event_code, List(false.B, false.B, false.B, false.B), Array(
        ...
        BitPat(2.U) -> List(false.B, false.B, true.B,  false.B),    // axi_busy          
        BitPat(3.U) -> List(false.B, false.B, true.B,  false.B)     // load_data_hit 
    ))
