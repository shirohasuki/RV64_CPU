...
class IDU extends Module {
    ...
    // IDU to EXU
    io.id_ex.inst_id_ex := io.if_id.inst_if_id
    ...
    var id_ex_list  = ListLookup(func37, List(0.U(64.W), 0.U(64.W), 0.U(5.W), 0.U(5.W), 0.U(64.W), false.B, 0.U(64.W), 0.U(64.W)), Array(
        BitPat("b000_0010011") -> List(io.reg_id.rs1_data_reg_id, immI,                      rs1_addr,       0.U,          rd_addr,    1.B,   0.U,   0.U), //addi
        BitPat("b000_0110011") -> List(io.reg_id.rs1_data_reg_id, io.reg_id.rs2_data_reg_id, rs1_addr,       rs2_addr,     rd_addr,    1.B,   0.U,   0.U), //add        
    ))
    io.id_ex.op1_id_ex         := id_ex_list(0)
    io.id_ex.op2_id_ex         := id_ex_list(1)
    io.id_ex.rs1_addr_id_ex    := id_ex_list(2)
    io.id_ex.rs2_addr_id_ex    := id_ex_list(3)
    io.id_ex.rd_addr_id_ex     := id_ex_list(4)
    io.id_ex.reg_wen_id_ex     := id_ex_list(5)
    io.id_ex.base_addr_id_ex   := id_ex_list(6)
    io.id_ex.offset_addr_id_ex := id_ex_list(7)
}
