...
class MCIF extends Module {
    ...
    val MCIF_R    = Module(new MCIF_R())
        ...
        if_mcif.mem_rdata := Mux(MCIF_R.resp0.valid, MCIF_R.resp0.bits, 0.U)
        MCIF_R.resp1.ready := 1.U
        ls_mcif.mem_rdata := Mux(MCIF_R.resp1.valid, MCIF_R.resp1.bits, 0.U)
    ...
    mcif_ctrl.axi_busy_end    := true.B//((mcif_axi_r.AXI_RID === 0.U) & mcif_axi_r.AXI_RLAST) | ((mcif_axi_w.AXI_AWID === 1.U) & mcif_axi_w.AXI_BRESP)
}
