...
class MEM extends Module {
    ...
    rdata := Mux(ren & (raddr =/= 0.U) & (raddr =/= "h80000000".U), mem.read(raddr >> 3), 
                Mux(raddr === "h80000000".U,  mem.read(raddr >> 3),0.U)) // 要提前读一次，过滤掉错误值，第一下默认返回mem.read(0)
    ...
    val DPIC_mtrace = Module(new mtrace())
        DPIC_mtrace.io.ren   := (rid === 0.U) & ren 
