...
object function {
    ...
    def SEXT (short: UInt): UInt = {
        ...
        SEXT_data := Cat(Fill(64-signedBitLength(short.getWidth), short(short.getWidth-1)), short(short.getWidth-1, 0))
        SEXT_data
    }
    def ZEXT (short: UInt): UInt = Cat(Fill(64-short.getWidth,   0.U), short(short.getWidth-1, 0)) // zero extension 瞎取的名字，向前补0
} 
