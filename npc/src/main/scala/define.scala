...
object function {
    ...
    // 这两个都是用在64位的，要拓展得改
    def SEXT (short: UInt): UInt = Cat(Fill(32, short(short.getWidth-1)), short(short.getWidth-1, 0))
    def ZEXT (short: UInt): UInt = Cat(Fill(32,   0.U), short(short.getWidth-1, 0)) // zero extension 瞎取的名字，向前补0
} 
