...
object function {
    ...
    def SEXT (short: UInt): UInt = Cat(Fill(64-int(short.getWidth), short(short.getWidth-1)), short(short.getWidth-1, 0))
    def ZEXT (short: UInt): UInt = Cat(Fill(32-int(short.getWidth),   0.U), short(short.getWidth-1, 0)) // zero extension 瞎取的名字，向前补0
} 
